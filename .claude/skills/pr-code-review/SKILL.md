...
2. **Placement:** Do the changes live in the correct place? This repo's
   layout:
   - `install-monokai-merge.ps1` and `install-monokai-merge.sh`: the two
     installers. They must stay behaviourally equivalent; a change to one
     almost always belongs in the other.
   - `tools/`: developer utilities, not part of the install path.
     `test-linux.sh` is the bash installer's only regression net.
   - `AGENTS.md`: findings, verification method, and the rules for changes.
   - `README.md`: user-facing install and uninstall documentation.
   Flag a change that lands in only one installer when both are affected, and
   any generated theme resource committed to the repository, which
...
These are correctness and safety rules for a pair of installers that write into
a user's Sublime Merge configuration. They are not style issues, and linters do
not catch them. `AGENTS.md` carries the reasoning; this list is the check.

- **Writing into a user's configuration.** An existing
  `Preferences.sublime-settings` is never rewritten. Flag any code that parses
  it and writes it back: Sublime settings legitimately contain `//` comments and
  trailing commas, so a parse-and-rewrite destroys settings on ordinary input.
  Flag equally any new write to a path under the user's `Packages/` that the
  installer does not also remove on `--uninstall`.
- **Installer parity.** The two installers must behave identically. Flag a
  change to one without the matching change to the other.
- **Uninstall symmetry.** Every file an installer creates must appear in its
  uninstall list. Flag a new output file that does not.
- **Idempotency.** Re-running an installer must be safe and must produce the
  same result. Flag anything that appends, or that depends on prior state.
- **Shell portability.** Flag interval expressions (`{0,4}`) in `awk`, which
  Debian's `mawk` does not support; `eval` on a value that can contain
  parentheses or spaces, such as `hsl(285, 5%, 17%)`; and a hard dependency on
  a tool where a fallback exists (`unzip` or `python3`; `git` only when the
  upstream clone is missing).
- **Line endings.** The shell scripts must be committed LF. Flag a change that
  weakens `.gitattributes`, and remember `core.autocrlf` is commonly enabled on
  Windows hosts.
- **Generated and third-party resources.** Flag any theme resource committed to
  the repository. `Merge Base.sublime-theme` is Sublime HQ's shipped file and is
  extracted on the target machine at run time; this repository redistributes
  nothing.
- **Generated JSON is validated before it is written.** Theme files permit `//`
  comments and trailing commas, so both must be stripped before parsing. Flag a
  write that is not gated on a validity check: a malformed theme fails silently
  and Merge falls back, which reads as "the rule did not work".
- **Colour claims are measured, not eyeballed.** Flag a claim that a surface is
  fixed which rests on a screenshot rather than pixel counts, and any
  verification that covers only one of the two view states (commit dialog and
  commit detail expose different surfaces).
